...
- **LevelUpModal** (`components/LevelUpModal.tsx`): celebrazione animata come esplosione cinematografica — (1) vecchio badge si proietta in avanti (scala 1→2.6 in 400ms) + fade-out; (2) flash bianco esplosivo (scala 5, 300ms) + particelle radiali; (3) nuovo badge spring-in energico (damping 5, stiffness 250) + coriandoli colorati + testo. Durata totale ~3.4s, tap per chiudere anticipatamente. `REVEAL_DELAY=600ms`, `AUTO_CLOSE_DELAY=3400ms`.
- **LevelUpProvider** (`context/level-up.tsx`): context globale in `_layout.tsx`. Persiste il livello precedente su AsyncStorage con chiave user-scoped (`leafy_prev_level:<userId>`). Rileva level-up sia dopo scan (via `checkForLevelUp()`) sia al ritorno in foreground (AppState). Si resetta al cambio utente. `WATERING_CAN_DURATION_MS=2700ms` (il modal si apre esattamente quando finisce l'annaffiatoio e il crescendo del badge raggiunge il picco).
...
- **Redesign estetico Shopkick-inspired** (Marzo 2026): Revisione visiva completa dell'UI mobile. Modifiche: (1) `RING_SIZE` 240→190, `RING_STROKE` 16→14, `ICON_BASE_SIZE` 90→72 — anello di livello più compatto e proporzionato; (2) Scan button tab bar ridotto da 79px a 64px outer, icona camera 30→24px; (3) Balance bar font 32→18, padding verticale ridotto; (4) Card check-in (classic e gold) — rimossi `LinearGradient` di background, rimossi cerchi decorativi (`decorCircle1/2/Gold1/Gold2`), rimossi progress bar, background bianco `#FFFFFF` con bordo neutro sottile, shadow `#000` neutrala (invece di colorata); (5) Palette stampe unificata al verde primario `#2E6B50` — `stampCellDone` `#0EA5E9`→`#2E6B50`, `stampCellNext` border `#38BDF8`→`#2E6B50`, future cells neutro `rgba(0,0,0,0.03)`, testi titolo/label `#0369A1`→`#2E6B50`; (6) Bottone check-in classic da `LinearGradient` a `View` con `backgroundColor: "#2E6B50"`, icona `water-plus`→`calendar-check`; (7) Bottone check-in gold da `LinearGradient` a `View` con `backgroundColor: "#D4A017"`; (8) `xpProgress` nel ring `#38BDF8`→verde mode-responsive; (9) `typeColor` sfide daily `#38BDF8`→`theme.leaf`; (10) heroSection background `#F2F9F5`→`theme.background` (neutralizza il tono mint nella home); (11) Marketplace background `#C8E6C9`→`theme.background`.
- **Check-in manuale** (`app/(tabs)/index.tsx`): La stampa card "Gocce Quotidiane" mostra un bottone "Fai Check In" / "Fatto oggi ✓" nella cella corrente (`isNext`). Al tap: (1) animazione spring bounce sulla cella (scala 1.4→1, `Animated.spring`); (2) radial confetti burst (`CheckinDropBurst` component — 12 particelle colorate con `useRef` Animated.Value[]; (3) bottone pulsa con `Animated.loop` breathing animation (scala 1→1.06→1, 900ms) quando non ancora completato oggi. Backend: `POST /api/profile/daily-checkin` già esistente; `lastLoginDate` esposto in `GET /api/profile`; `checkedInToday` derivato client-side confrontando `lastLoginDate` con today UTC. File: `components/CheckinDropBurst.tsx`.
...
✅ **Task #1–#2 MERGED (sessione 24/03/2026)**
- Task #1: Check-in manuale — bottone "Fai Check In", bounce stamp cell, confetti burst `CheckinDropBurst`
- Task #2: Animazione level-up — crescendo badge (11 cicli ±2%→±18%), `displayedLevel` swap, esplosione cinematografica in `LevelUpModal`
